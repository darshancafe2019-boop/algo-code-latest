...
def place_order():
    ...
    exec_res = execution_service.execute_order(
        ...
        stop_loss=validated.get("stop_loss", validated.get("stopLoss", 0.0)),
        take_profit=validated.get(
            "take_profit",
            validated.get("takeProfit", validated.get("target", 0.0)),
        ),
        confidence_score=validated.get("confidence_score", validated.get("confidence", 85.0)),
        market_tick_iso=validated.get("market_tick_iso") or validated.get("market_timestamp"),
        account_balance=validated.get("account_balance", 50000.0),
        mode=mode,
