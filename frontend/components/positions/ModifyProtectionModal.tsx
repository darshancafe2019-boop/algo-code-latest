...
import { X, Shield, Target, Sliders, AlertTriangle, CheckCircle2, DollarSign } from "lucide-react";
...
export function ModifyProtectionModal({
  ...
  const modifyMutation = useMutation({
    mutationFn: async () => {
      if (!position) return;
      const res = await fetch(`/api/positions/${position.id}/modify-protection`, {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "Failed to modify protection");
      return data;
    },
    onSuccess: () => {
  ...
  const plannedRisk = Math.abs(entryP - stopLoss) * qty;
  const plannedReward = Math.abs(takeProfit - entryP) * qty;
  const rrRatio = plannedRisk > 0 ? (plannedReward / plannedRisk).toFixed(2) : "—";
  const slDistPct = entryP > 0 ? Math.abs(((stopLoss - entryP) / entryP) * 100).toFixed(2) : "0.00";
  const tpDistPct = entryP > 0 ? Math.abs(((takeProfit - entryP) / entryP) * 100).toFixed(2) : "0.00";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150 font-sans select-none">
      <div className="w-full max-w-md bg-[var(--theme-surface)] border border-[var(--theme-border)] rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-[var(--theme-text-primary)]">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="p-3 rounded-xl bg-[var(--theme-elevated)] border border-[var(--theme-border-subtle)] flex items-center justify-between text-xs font-mono">
            <div>
            ...
            <div className="text-right">
              ...
              <span className="font-bold text-[var(--theme-accent)] tabular-nums">
                ${Number(position.current_price || entryP).toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-[10px] font-mono text-[var(--theme-loss)]">
                -{slDistPct}% (${plannedRisk.toFixed(2)} Risk)
              </span>
            </div>
            <input
              type="number"
              ...
              className="w-full px-3 py-2 bg-[var(--theme-elevated)] border border-[var(--theme-border-subtle)] rounded-xl text-sm font-mono font-bold text-[var(--theme-text-primary)] focus:outline-none focus:border-[var(--theme-loss)] transition"
            />
            {/* Quick Presets */}
            <div className="flex items-center gap-1.5 text-[10px] font-mono">
              ...
              {[1.0, 1.5, 2.0, 3.0, 5.0].map((pct) => (
                <button
                  ...
                  className="px-2 py-0.5 rounded bg-[var(--theme-elevated)] hover:bg-[var(--theme-loss)]/20 text-[var(--theme-text-secondary)] hover:text-[var(--theme-loss)] border border-[var(--theme-border-subtle)] transition"
                >
                  -{pct}%
                </button>
              ))}
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-[10px] font-mono text-[var(--theme-profit)]">
                +{tpDistPct}% (${plannedReward.toFixed(2)} Reward)
              </span>
            </div>
            <input
              type="number"
              ...
              className="w-full px-3 py-2 bg-[var(--theme-elevated)] border border-[var(--theme-border-subtle)] rounded-xl text-sm font-mono font-bold text-[var(--theme-text-primary)] focus:outline-none focus:border-[var(--theme-profit)] transition"
            />
            {/* Quick Presets */}
            <div className="flex items-center gap-1.5 text-[10px] font-mono">
              <span className="text-[var(--theme-text-muted)]">Presets:</span>
              {[2.0, 3.0, 4.0, 6.0, 8.0].map((pct) => (
                <button
                  ...
                  className="px-2 py-0.5 rounded bg-[var(--theme-elevated)] hover:bg-[var(--theme-profit)]/20 text-[var(--theme-text-secondary)] hover:text-[var(--theme-profit)] border border-[var(--theme-border-subtle)] transition"
                >
                  +{pct}%
                </button>
              ))}
          ...
          <div className="p-3 bg-[var(--theme-elevated)]/60 border border-[var(--theme-border-subtle)] rounded-xl text-xs font-mono flex items-center justify-between">
            <div>
              <span className="text-[10px] text-[var(--theme-text-muted)] block">Planned R:R Ratio</span>
              <span className="font-extrabold text-[var(--theme-accent)]">1 : {rrRatio}</span>
            </div>
            <div className="text-right">
        ...
        <div className="p-5 border-t border-[var(--theme-border)] bg-[var(--theme-elevated)] flex items-center justify-end gap-2.5">
          ...
          <button
            ...
            className="px-5 py-2 rounded-xl bg-[var(--theme-accent)] text-[var(--theme-bg)] font-bold text-xs shadow-md transition disabled:opacity-50 flex items-center gap-1.5"
          >
