  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans select-none">
      {positions.map((pos) => {
        ...
        const slP = Number(pos.stop_loss || entryP * 0.98);
        const tpP = Number(pos.take_profit || entryP * 1.04);
        ...
        const minP = Math.min(slP, entryP, currP);
        const maxP = Math.max(tpP, entryP, currP);
        ...
        return (
          <div
            ...
            className="p-4 sm:p-5 rounded-2xl bg-[var(--theme-surface)] border border-[var(--theme-border)] hover:border-[var(--theme-accent)]/50 transition-all shadow-lg flex flex-col justify-between space-y-4 cursor-pointer group"
          >
            {/* Top Row: Symbol & P&L */}
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-xs text-[var(--theme-text-secondary)] mt-0.5">
                  {pos.bot_name || pos.bot_id || "Bot-1"} • {pos.strategy || "Strategy"}
                </div>
              </div>

              <div className="text-right font-mono tabular-nums">
                <div
                  className={`text-lg font-extrabold flex items-center justify-end gap-0.5 ${
            ...
            <div className="p-3 bg-[var(--theme-elevated)] border border-[var(--theme-border-subtle)] rounded-xl space-y-2.5 font-mono text-xs">
              {/* Relative Track Bar */}
              <div className="relative h-2 w-full bg-[var(--theme-surface)] rounded-full my-3.5 border border-[var(--theme-border-subtle)]">
                ...
                <div
                  className="absolute top-1/2 -translate-y-1/2 left-0 w-2.5 h-2.5 bg-[var(--theme-loss)] rounded-full ring-2 ring-[var(--theme-surface)]"
                  title={`Stop Loss: $${slP.toLocaleString()}`}
                />

                {/* Entry Price Marker */}
                <div
                  className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-[var(--theme-text-muted)] rounded-full ring-2 ring-[var(--theme-surface)]"
                  style={{ left: `${entryPct}%` }}
                ...
                <div
                  className={`absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full ring-2 ring-[var(--theme-surface)] transition-all duration-300 ${
                    isProfit
                      ? "bg-[var(--theme-profit)] shadow-md shadow-[var(--theme-profit)]/50"
                      : "bg-[var(--theme-loss)] shadow-md shadow-[var(--theme-loss)]/50"
                  }`}
                ...
                <div
                  className="absolute top-1/2 -translate-y-1/2 right-0 w-2.5 h-2.5 bg-[var(--theme-profit)] rounded-full ring-2 ring-[var(--theme-surface)]"
                  title={`Take Profit: $${tpP.toLocaleString()}`}
                />
              </div>

              {/* Numerical Price Points Strip */}
              <div className="grid grid-cols-4 gap-1 text-[10px] pt-1">
                <div>
                  <span className="text-[var(--theme-loss)] uppercase flex items-center gap-0.5">
                ...
                <div>
                  <span className="text-[var(--theme-text-muted)] uppercase">ENTRY</span>
                  <span className="text-[var(--theme-text-primary)] font-bold block tabular-nums">
                ...
                <div>
                  <span className="text-[var(--theme-accent)] uppercase">MARK</span>
                  <span
                    className={`font-extrabold block tabular-nums ${
                      isProfit ? "text-[var(--theme-profit)]" : "text-[var(--theme-loss)]"
                    }`}
                  >
                ...
                <div className="text-right">
                  <span className="text-[var(--theme-profit)] uppercase flex items-center justify-end gap-0.5">
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-xl bg-[var(--theme-elevated)] hover:bg-[var(--theme-warning)]/20 text-[var(--theme-text-secondary)] hover:text-[var(--theme-warning)] border border-[var(--theme-border-subtle)] text-xs font-bold transition"
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-[var(--theme-loss)]/15 hover:bg-[var(--theme-loss)] text-[var(--theme-loss)] hover:text-white border border-[var(--theme-loss)]/30 text-xs font-bold font-mono transition"
