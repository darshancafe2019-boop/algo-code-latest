  ...
  return (
    <div className="space-y-4 font-sans select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 text-xs font-mono">
        ...
        <div className="lg:col-span-7 bg-[#0A1422] border border-[#122033] rounded-2xl p-4 space-y-3.5">
          {/* Asset Class Selector */}
          <div className="space-y-1">
            <label className="text-[10px] text-[#52627A] font-bold block">Asset Class</label>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] text-[#52627A] font-bold block">Total Capital ($)</label>
              <input
                type="number"
                value={capital}
                onChange={(e) => setCapital(parseFloat(e.target.value) || 0)}
                className="w-full bg-[#07101A] border border-[#122033] rounded-lg p-2 text-white font-bold focus:outline-none focus:border-[#22D3EE]"
              />
            </div>

            <div>
              <label className="text-[10px] text-[#52627A] font-bold block">Risk Per Trade (%)</label>
              <input
                type="number"
                step={0.1}
                value={riskPct}
                onChange={(e) => setRiskPct(parseFloat(e.target.value) || 0)}
                className="w-full bg-[#07101A] border border-[#122033] rounded-lg p-2 text-cyan-300 font-bold focus:outline-none focus:border-[#22D3EE]"
              />
            </div>

            <div>
              <label className="text-[10px] text-[#52627A] font-bold block">Leverage Multiplier</label>
              <input
                type="number"
                value={leverage}
                onChange={(e) => setLeverage(parseFloat(e.target.value) || 1)}
                className="w-full bg-[#07101A] border border-[#122033] rounded-lg p-2 text-purple-300 font-bold focus:outline-none focus:border-[#22D3EE]"
              />
            </div>
            ...
            <div>
              <label className="text-[10px] text-[#52627A] font-bold block">Stop Loss Price ($)</label>
              <input
                type="number"
                value={stopLoss}
                onChange={(e) => setStopLoss(parseFloat(e.target.value) || 0)}
                className="w-full bg-[#07101A] border border-[#122033] rounded-lg p-2 text-red-400 font-bold focus:outline-none focus:border-red-500"
              />
            </div>

            <div>
              <label className="text-[10px] text-[#52627A] font-bold block">Target Price ($)</label>
              <input
                type="number"
                value={targetPrice}
                onChange={(e) => setTargetPrice(parseFloat(e.target.value) || 0)}
                className="w-full bg-[#07101A] border border-[#122033] rounded-lg p-2 text-[#22D3EE] font-bold focus:outline-none focus:border-emerald-500"
              />
            </div>
          </div>
        ...
        <div className="lg:col-span-5 bg-[#0A1422] border border-[#122033] rounded-2xl p-4 space-y-3 flex flex-col justify-between">
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-white uppercase tracking-wider border-b border-[#122033] pb-2 flex items-center justify-between">
              ...
              <span className="text-[10px] text-[#22D3EE]">Fixed Fractional</span>
            </h4>
            ...
            {calcResult ? (
              <div className="space-y-2.5 text-xs animate-fadeIn">
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033]">
                    <span className="text-[10px] text-[#52627A] block">Target Quantity</span>
                    <span className="text-sm font-bold text-[#22D3EE]">{calcResult.position_quantity} Units</span>
                  </div>

                  <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033]">
                    <span className="text-[10px] text-[#52627A] block">Max Risk Dollars</span>
                  ...
                  <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033]">
                    <span className="text-[10px] text-[#52627A] block">Required Margin</span>
                    <span className="text-sm font-bold text-cyan-300">${calcResult.margin_required.toFixed(2)}</span>
                  </div>

                  <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033]">
                    <span className="text-[10px] text-[#52627A] block">Potential Profit</span>
                  ...
                  <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033]">
                    <span className="text-[10px] text-[#52627A] block">Risk / Reward (R:R)</span>
              ...
              <div className="p-8 text-center text-[#52627A] text-xs">
                Configure parameters and click calculate to compute risk-adjusted sizing.
              </div>
            )}
          </div>

          <p className="text-[10px] text-[#52627A] font-sans pt-2 border-t border-[#122033]">
