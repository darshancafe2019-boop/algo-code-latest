...
export function TopGlobalRiskBar({ overview, onOpenEmergencyModal }: TopGlobalRiskBarProps) {
  ...
  return (
    <div className="bg-[#0A1422] border border-[#1A2A3F] rounded-2xl p-4 sm:p-5 shadow-2xl select-none font-sans space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3.5 min-w-[280px]">
          <div
            className={`p-3 rounded-2xl border shadow-lg flex items-center justify-center ${
              isHalted
                ...
                : "bg-gradient-to-tr from-[rgba(37,99,235,0.18)] to-[#2563EB] text-[#22D3EE] border-[#00E890]/40 shadow-[#2563EB]/20"
            }`}
          >
            {isHalted ? (
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#7C8CA3] font-bold uppercase tracking-wider">
                Risk Status:
              </span>
              <span
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-[#52627A] font-mono">/ 100 Portfolio Risk Score</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs font-mono">
          ...
          {/* Margin */}
          <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033] space-y-0.5">
            <span className="text-[10px] text-[#52627A] uppercase font-bold block">Margin Used</span>
            <span className="text-sm font-bold text-purple-300">{marginPct.toFixed(1)}%</span>
            <span className="text-[10px] text-[#22D3EE] block">{(100 - marginPct).toFixed(1)}% Free</span>
          </div>

          {/* Daily Loss */}
          <div className="p-2.5 rounded-xl bg-[#07101A] border border-[#122033] space-y-0.5">
            <span className="text-[10px] text-[#52627A] uppercase font-bold block">Daily Loss Buffer</span>
            <span className="text-sm font-bold text-[#22D3EE]">{dailyLossRemaining.toFixed(1)}% left</span>
            <span className="text-[10px] text-[#52627A] block">{dailyLossPct.toFixed(1)}% / {maxDailyLossLimit}% Limit</span>
          </div>
        </div>

        {/* Pre-Order Gate & Emergency Kill Switch Trigger */}
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#07101A] border border-emerald-800/40 text-xs font-mono text-[#22D3EE]">
            <CheckCircle2 className="h-4 w-4" />
            <div>
              <span className="font-bold block">14 / 14 PASS</span>
              <span className="text-[9px] text-[#52627A] uppercase">Pre-Order Gates</span>
            </div>
          </div>
      ...
      <div className="pt-3 border-t border-[#122033] grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2 text-[11px] font-mono">
        <div className="p-2 rounded-lg bg-[#07101A] border border-[#122033]">
          <span className="text-[#52627A] block text-[9px] uppercase font-bold">1. Can I Trade?</span>
          <span className="text-[#22D3EE] font-bold">YES (GATES OPEN)</span>
        </div>
        ...
        <div className="p-2 rounded-lg bg-[#07101A] border border-[#122033]">
          <span className="text-[#52627A] block text-[9px] uppercase font-bold">4. Max Risk/Trade?</span>
        ...
        <div className="p-2 rounded-lg bg-[#07101A] border border-[#122033]">
          <span className="text-[#52627A] block text-[9px] uppercase font-bold">6. Halting Limit?</span>
          <span className="text-amber-400 font-bold">-$500 (5.0% DD)</span>
        </div>

        <div className="p-2 rounded-lg bg-[#07101A] border border-[#122033]">
          <span className="text-[#52627A] block text-[9px] uppercase font-bold">7. Last Rejection?</span>
          <span className="text-[#22D3EE] font-bold">NONE (0 Blocked)</span>
        </div>
      </div>
    </div>
  );
