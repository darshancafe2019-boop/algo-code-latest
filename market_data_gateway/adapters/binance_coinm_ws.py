"""
Binance COIN-M Futures WebSocket Adapter
=========================================
Dedicated, auto-reconnecting WebSocket adapter for Binance Coin-Margined (Inverse) Futures.
Connects to wss://dstream.binance.com/stream for public market data:
- <symbol>@ticker (24hr rolling window ticker statistics)
- <symbol>@markPrice@1s (Mark price and funding rate / basis)
- <symbol>@depth20@100ms (Top 20 order book levels)
...
logger = logging.getLogger("MDGateway.BinanceCOINM")

BINANCE_COINM_WS_BASE = "wss://dstream.binance.com/stream"
BINANCE_COINM_REST_BASE = "https://dapi.binance.com/dapi/v1"
MAX_STREAMS_PER_CONNECTION = 200
MAX_BACKOFF_SEC = 60.0

DEFAULT_COINM_SYMBOLS = [
    "BTCUSD_PERP",
    "ETHUSD_PERP",
    "SOLUSD_PERP",
    "BNBUSD_PERP",
...
def _canonical_to_coinm(symbol: str) -> str:
    """Convert 'BTC/USD:BTC' or 'BINANCE:BTCUSD_PERP' -> 'btcusd_perp'."""
    clean = symbol.strip().upper()
    if clean.startswith("BINANCE:"):
        clean = clean[len("BINANCE:"):]
    if clean.endswith(":BTC") or clean.endswith(":ETH") or clean.endswith(":SOL") or clean.endswith(":COINM"):
        clean = clean.split(":")[0]
    clean = clean.replace("/", "").replace("-", "")
    if not clean.endswith("_PERP") and clean.endswith("USD"):
        clean = f"{clean}_PERP"
    return clean.lower()


def _coinm_to_canonical(raw_symbol: str) -> str:
    """Convert 'BTCUSD_PERP' -> 'BTC/USD:BTC'."""
    s = raw_symbol.upper()
    base = s.replace("_PERP", "").replace("USD", "")
    return f"{base}/USD:{base}"


class BinanceCoinMWSAdapter(BaseProviderAdapter):
    """
    Persistent Binance COIN-M Delivery and Inverse Perpetual Futures adapter.
    """

    def __init__(self):
        super().__init__("binance_coinm", "Binance COIN-M Futures")
        self._ws = None
    ...
    async def connect(self) -> None:
        ...
        self._running = True
        self._status = "LIVE"
        logger.info("BinanceCoinMWSAdapter initialized.")
    ...
    async def disconnect(self) -> None:
        ...
        if self._ws:
            try:
                ...
                pass
        self._status = "DISCONNECTED"
        logger.info("BinanceCoinMWSAdapter disconnected.")

    async def subscribe(self, symbols: List[str]) -> None:
        new_streams = set()
        for s in symbols:
            clean = _canonical_to_coinm(s)
            if clean:
        ...
        if self._ws and not self._ws.closed and new_streams:
            ...
            try:
                ...
                logger.warning(f"Error subscribing on active COIN-M WS: {e}")
    ...
    async def unsubscribe(self, symbols: List[str]) -> None:
        ...
        if self._ws and not self._ws.closed and unsub_streams:
            ...
            try:
                ...
                logger.warning(f"Error unsubscribing on COIN-M WS: {e}")

    async def get_snapshot(self, symbols: List[str]) -> Dict[str, NormalizedQuote]:
        result = {}
        for s in symbols:
            clean = _canonical_to_coinm(s).upper()
            canonical_id = _coinm_to_canonical(clean)
    ...
    async def get_instruments(self) -> List[CanonicalInstrument]:
        instruments = []
        for sym in DEFAULT_COINM_SYMBOLS:
            base = sym.replace("USD_PERP", "").replace("USD", "")
            canonical_sym = _coinm_to_canonical(sym)
            instruments.append(
                CanonicalInstrument(
                    canonical_symbol=canonical_sym,
                    display_name=f"{base}/USD Coin-Margined Perpetual",
                    asset_class="Crypto_Inverse",
                    exchange="BINANCE",
                    mic_code="BNCF",
                    region="GLOBAL",
                    currency="USD",
                    timezone="UTC",
                    tick_size=0.1 if "BTC" in sym else 0.01,
                    lot_size=1.0,
                )
            )
        return instruments
    ...
    async def _run_ws_loop(self) -> None:
        while self._running:
            try:
                streams = set()
                targets = self._subscribed_symbols if self._subscribed_symbols else DEFAULT_COINM_SYMBOLS
                for s in targets:
                    clean = _canonical_to_coinm(s)
                    if clean:
                ...
                uri = f"{BINANCE_COINM_WS_BASE}?streams={stream_path}"

                logger.info(f"Connecting to Binance COIN-M WS: {len(streams)} streams")
                async with websockets.connect(uri, ping_interval=20, ping_timeout=10) as ws:
                    self._ws = ws
                    self._status = "LIVE"
                    self._retry_count = 0
                    logger.info("Binance COIN-M WebSocket connected.")
                    ...
                    async for raw_msg in ws:
                        ...
                        try:
                            ...
                            logger.debug(f"Error handling COIN-M stream payload: {e}")
                ...
                logger.warning(f"Binance COIN-M WS error: {e}. Reconnecting in {backoff:.1f}s...")
    ...
    def _handle_stream_message(self, msg: Dict[str, Any]) -> None:
        ...
        if e_type == "24hrTicker" or "@ticker" in stream:
            ...
            canonical_sym = _coinm_to_canonical(raw_sym)
