    ...
    with _db_init_lock:
        ...
        for attempt in range(5):
            try:
                ...
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_signals_log_sym_ts ON signals_log(symbol, timestamp DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_signals_log_ts ON signals_log(timestamp DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_bot_instances_sym ON bot_instances(symbol)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_positions_bot_status ON positions(bot_id, status)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_positions_sym ON positions(symbol)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_bot_activity_logs_bot_ts ON bot_activity_logs(bot_id, timestamp DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_bot_decision_logs_bot_ts ON bot_decision_logs(bot_id, timestamp DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_system_errors_status_occ ON system_errors(status, occurrence_count DESC)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_system_errors_ts ON system_errors(timestamp DESC)")
