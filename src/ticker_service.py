            ...
            try:
                ...
                if last_price > 0:
                    payload = {
                        ...
                        "volume": float(raw.get("baseVolume") or raw.get("quoteVolume") or 0.0) if (raw.get("baseVolume") is not None or raw.get("quoteVolume") is not None) else None,
                        "change_pct": float(raw["percentage"]) if raw.get("percentage") is not None else None,
                        "change_val": float(raw["change"]) if raw.get("change") is not None else None,
    ...
    def _build_fallback_response(self, symbol: str, reason: str, start_perf: float) -> Dict[str, Any]:
        ...
        if db_candle:
            ...
            return {
                ...
                "last": last_p,
                "price": last_p,
                "high": float(db_candle["high"]) if db_candle.get("high") is not None else None,
                "low": float(db_candle["low"]) if db_candle.get("low") is not None else None,
                "volume": float(db_candle["volume"]) if db_candle.get("volume") is not None else None,
                "change_pct": None,
                "change_val": None,
                "bid": None,
                "ask": None,
