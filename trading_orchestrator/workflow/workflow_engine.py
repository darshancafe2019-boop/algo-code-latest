...
and dispatches real-time events to connected subscribers (SSE/WebSocket).
"""
...
from trading_orchestrator.scheduler.checkpoints import DEFAULT_CHECKPOINTS

logger = logging.getLogger("WorkflowEngine")


class WorkflowEngine:
    """Central engine driving automated trading workflows."""
    ...
    def trigger_checkpoint_execution(
        ...
        """
        Executes a complete checkpoint workflow through the state machine.
        """
        with self._lock:
            if self._is_killed:
            ...
            if self._is_paused:
                logger.info("[WORKFLOW] Orchestrator is PAUSED. Skipping checkpoint %s", checkpoint_id)
                return {"status": "SKIPPED", "reason": "ORCHESTRATOR_PAUSED"}

            run_id = f"RUN-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
            checkpoint_cfg = DEFAULT_CHECKPOINTS.get(checkpoint_id)
            checkpoint_name = checkpoint_cfg.name if checkpoint_cfg else checkpoint_id
            ...
            self.broadcast_event("CHECKPOINT_STARTED", {"run_id": run_id, "checkpoint_id": checkpoint_id, "state": "RUNNING"})
        ...
        try:
            ...
            # 6. State: COMPLETED
            global_workflow_state_mgr.transition_to(WorkflowState.COMPLETED, f"Checkpoint {checkpoint_id} completed successfully")
            self.broadcast_event("CHECKPOINT_COMPLETED", {"run_id": run_id, "checkpoint_id": checkpoint_id, "result": result})
            ...
            db.safe_execute(
                ...
                WHERE checkpoint_id = ?
                """,
                (now_iso, result.get("summary", "Completed"), checkpoint_id)
            )

            return result

        except Exception as e:
            logger.error("[WORKFLOW_ERROR] Checkpoint %s failed: %s", checkpoint_id, e)
            try:
                global_workflow_state_mgr.transition_to(WorkflowState.FAILED, f"Error: {e}")
            except Exception:
                pass
            self.broadcast_event("CHECKPOINT_FAILED", {"run_id": run_id, "checkpoint_id": checkpoint_id, "error": str(e)})
