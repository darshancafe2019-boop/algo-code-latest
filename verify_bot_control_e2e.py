...
def test_bots_summary():
    ...
    res = requests.get(f"{BASE_URL}/api/bots/summary", timeout=10)
...
def test_bots_fleet_listing():
    ...
    res = requests.get(f"{BASE_URL}/api/bots", timeout=10)
    assert res.status_code == 200, f"Expected 200, got {res.status_code}"
    data = res.json()
    assert data.get("status") == "success"
    bots = data.get("bots", [])
...
def test_idempotent_command_execution(bot_id):
    ...
    res1 = requests.post(f"{BASE_URL}/api/bots/command", json=payload, timeout=10)
    ...
    res2 = requests.post(f"{BASE_URL}/api/bots/command", json=payload, timeout=10)
...
def test_signal_debugger(bot_id):
    ...
    res = requests.get(f"{BASE_URL}/api/bots/{bot_id}/signal-debugger", timeout=15)
    assert res.status_code == 200, f"Expected 200, got {res.status_code}"
    data = res.json()
    assert data.get("status") == "success"
...
def test_worker_leases_and_state_transitions(bot_id):
    ...
    res_stop = requests.post(f"{BASE_URL}/api/bots/command", json={
        "command_id": stop_cmd,
        "bot_id": bot_id,
        "action": "STOP"
    }, timeout=10)
    ...
    res_resume = requests.post(f"{BASE_URL}/api/bots/command", json={
